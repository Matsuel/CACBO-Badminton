...
const Button = ({
    ...
    return (
        <Link href={href} target={target}>
            <button className={`flex ${rowReverse ? "flex-row-reverse" : "flex-row"} items-center justify-center gap-2 rounded-ten px-horizontal py-vertical font-Montserrat font-medium text-xl ${buttonStyle[variant]} ${className}`}>
                {icon}
                {children}
            </button>
        </Link>
