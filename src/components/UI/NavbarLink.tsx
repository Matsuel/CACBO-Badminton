...
const NavbarLink = ({ title, href, description, icon }: NavbarLinkType) => {
	return (
		<Link
			href={href}
		...
		>
			...
			<div className="w-[65%] h-auto flex flex-col gap-3 items-start">
				<h3 className={textVariants.h3}>{title}</h3>
				{description && (
					<p className={`${textVariants.secondary} w-3/4`}>{description}</p>
				)}
			</div>
